...
class MonteCarloSimApp(ft.UserControl):
    def initialize_tabs(self):
        ...
        self.tab1_controls = ft.Column(
            controls=[
                ft.Row(controls=[self.num_simulations, self.backlog_length, self.focus_percent, self.start_date]),
                ft.Row(controls=[self.select_throughput_btn, self.select_leadtimes_btn, self.run_simulation_btn]),
                ft.Row(controls=[self.download_table_button, self.download_insights_button, self.download_plot_button]),
                ft.Row(controls=[self.forecast_table_container, self.plot_image]),
                self.trend_insight_label, 
                self.volatility_insight_label, 

                self.correlation_insight_label,
            ],
            expand=True,
        ...
        self.tab2_controls = ft.Column(
            controls=[
                ft.Row(controls=[self.num_simulations_sp, self.backlog_length_sp, self.focus_percent_sp, self.start_date_sp]),
                ft.Row(controls=[self.select_story_point_btn, self.run_simulation_btn_sp]),
                ft.Row(controls=[self.download_table_button_sp, self.download_insights_button_sp, self.download_plot_button_sp]),
    ...
    def __init__(self, page):
        ...
        self.correlation_insight_label = ft.Text("")
        ...
        def save_insights_result(e: ft.FilePickerResultEvent):
            if e.path:
                insights_text = (
                    ...
                    "\n\nCorrelation Analysis:\n" + self.correlation_insight_label.value
                )
    ...
    def analyze_volatility(self,throughput_data, leadtimes_data):
        ...
        return insight
    def analyze_correlation(self,throughput_data, leadtimes_data):
        correlation = throughput_data['Throughput'].corr(leadtimes_data['Leadtime'])

        insight = f"The correlation between throughput and lead times is {correlation:.2f}."
        return insight
    ...
    def run_simulation(self, e):
        ...
        self.correlation_insight_label.value = self.analyze_correlation(throughput_data, leadtimes_data)
